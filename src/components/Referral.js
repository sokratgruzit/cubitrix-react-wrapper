...
const Referral = () => {
  ...
  const getOptions = async (type) => {
    console.log('asdasbdjhabshdbasjdhashd')
    setLvlData([]);
    try {
      ...
      console.log(data.object_value,'yle')
      console.log('asdasbdjhabshdbasjdhashd')
    } catch (err) {
      console.log(err);
    }
  ...
  let lvlTableType = (
      <div className={`referral-inner-table-more`}>
        <div
            ...
            onClick={() => {setLvlType("uni"); getOptions('Uni')}}>
          Uni
        </div>
        <div
            className={`referral-table-more-svg ${
            ...
            onClick={() => {setLvlType("binary"); getOptions('Binary bv')}}>
          Binary
        </div>
      </div>
  );
  ...
  return (
    ...
    />
        ...
        {levelSystemPopupActive && (
            <Popup
                popUpElement={
                  <LevelSystem tableHead={lvlType == 'uni' ? uniTh : binaryTh} tableData={lvlType == 'uni' && lvlData ? lvlData :  lvlData?.options} bv={lvlType == 'binary' && lvlData?.bv} mobile={false} type={lvlType} typeBns={lvlTableType} />
                }
